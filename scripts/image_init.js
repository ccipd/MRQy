...
function enter_select_image_view (dir) {
	...
	for (var j = 0; j < ORIGINAL_DATASET.length; j ++) {
		if (patient_names[j] == dir){
			...
			for (var i = 1; i < ORIGINAL_DATASET[j]["Number"]+1; i++) {
				$div.append("<img id='exibit-img' src='" + generate_img_src(dir, CURRENT_IMAGE_TYPE,image_names_new[i])+ "'/>");
			}
		}
	}
	...
	// for (var j = 0; j < ORIGINAL_DATASET.length; j++){
	// 	var image_names_new = image_names[j].split(re);
		// for (var i = 1; i < ORIGINAL_DATASET[j]["Number of Images"]+1; i++) {
		// 	$div.append("<img id='exibit-img' src='" + generate_img_src(dir, CURRENT_IMAGE_TYPE,image_names_new[i])+ "'/>");
		// }
	// }
	...
	// console.log(image_names[0].split(re))
	// console.log(image_names[0].split(re).length)
	// console.log(ORIGINAL_DATASET[0]["Number of Images"])
	// console.log(ORIGINAL_DATASET[0]["Number of Images"]+1)
	// console.log(image_names[1].split(re))
	// console.log(image_names[1].split(re).length)
	// console.log(ORIGINAL_DATASET[1]["Number of Images"])
...
// 					"<img src='" + generate_img_src(file_name, img_type) + "' file_name='" + file_name + "' img_type='" + img_type + "' onerror=\"this.style.display='none'\"/>";
// 	if (compare_type != -1) {
// 		img_block += "<img src='" + generate_img_src(file_name, compare_type) + "' file_name='" + file_name + "' img_type='" + compare_type + "' onerror=\"this.style.display='none'\"/>";
// 	}
